...
func partone() int {
	...
	for fileScanner.Scan() {
		...
		if firstSectionFirst >= secondSectionFirst && firstSectionSecond <= secondSectionSecond {
			counter++
			continue
		}
		if secondSectionFirst >= firstSectionFirst && secondSectionSecond <= firstSectionSecond {
			counter++
			continue
		}
	}
