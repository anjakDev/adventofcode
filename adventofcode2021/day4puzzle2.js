...
rl.on('close', () => {
        ...
        for (let j = 0; j < boards.length; j++) {
            ...
            if (idxRow !== -1) {
                ...
                if (sumRow === 5 || sumColumn === 5) {
                    boardEliminationArray.push(j);
                    if (boardEliminationArray.length === boards.length) {
    ...
    let sumUnmarked = 0;
    let finalMarked = marked[boardEliminationArray.at(boardEliminationArray.length - 1)];
    for (let i = 0; i < finalMarked.length; i++) {
        for (let j = 0; j < finalMarked[i].length; j++) {
            if (finalMarked[i][j] === 0) {
                sumUnmarked += boards[boardEliminationArray.at(boardEliminationArray.length - 1)][i][j]
            }
        }
    }
